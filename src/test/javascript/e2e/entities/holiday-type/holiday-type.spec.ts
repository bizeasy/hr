...
describe('HolidayType e2e test', () => {
  ...
  it('should load HolidayTypes', async () => {
    ...
    expect(await holidayTypeComponentsPage.getTitle()).to.eq('hrApp.holidayType.home.title');
    await browser.wait(
  ...
  it('should load create HolidayType page', async () => {
    ...
    expect(await holidayTypeUpdatePage.getPageTitle()).to.eq('hrApp.holidayType.home.createOrEditLabel');
  ...
  it('should create and save HolidayTypes', async () => {
    ...
    await promise.all([holidayTypeUpdatePage.setNameInput('name'), holidayTypeUpdatePage.setDescriptionInput('description')]);

    expect(await holidayTypeUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await holidayTypeUpdatePage.getDescriptionInput()).to.eq(
      'description',
  ...
  it('should delete last HolidayType', async () => {
    ...
    expect(await holidayTypeDeleteDialog.getDialogTitle()).to.eq('hrApp.holidayType.delete.question');
