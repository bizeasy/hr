...
describe('FacilityGroup e2e test', () => {
  ...
  it('should load FacilityGroups', async () => {
    ...
    expect(await facilityGroupComponentsPage.getTitle()).to.eq('hrApp.facilityGroup.home.title');
    await browser.wait(
  ...
  it('should load create FacilityGroup page', async () => {
    ...
    expect(await facilityGroupUpdatePage.getPageTitle()).to.eq('hrApp.facilityGroup.home.createOrEditLabel');
  ...
  it('should create and save FacilityGroups', async () => {
    ...
    await promise.all([
      facilityGroupUpdatePage.setNameInput('name'),
      facilityGroupUpdatePage.setDescriptionInput('description'),
      facilityGroupUpdatePage.facilityGroupTypeSelectLastOption(),
      facilityGroupUpdatePage.facilityGroupSelectLastOption(),
    ]);

    expect(await facilityGroupUpdatePage.getNameInput()).to.eq('name', 'Expected Name value to be equals to name');
    expect(await facilityGroupUpdatePage.getDescriptionInput()).to.eq(
      'description',
  ...
  it('should delete last FacilityGroup', async () => {
    ...
    expect(await facilityGroupDeleteDialog.getDialogTitle()).to.eq('hrApp.facilityGroup.delete.question');
