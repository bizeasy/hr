...
describe('Status e2e test', () => {
  ...
  it('should load Statuses', async () => {
    ...
    expect(await statusComponentsPage.getTitle()).to.eq('hrApp.status.home.title');
  ...
  it('should load create Status page', async () => {
    ...
    expect(await statusUpdatePage.getPageTitle()).to.eq('hrApp.status.home.createOrEditLabel');
  ...
  it('should create and save Statuses', async () => {
    ...
    await promise.all([
      ...
      statusUpdatePage.setActionInput('action'),
    ...
    expect(await statusUpdatePage.getActionInput()).to.eq('action', 'Expected Action value to be equals to action');
  ...
  it('should delete last Status', async () => {
    ...
    expect(await statusDeleteDialog.getDialogTitle()).to.eq('hrApp.status.delete.question');
