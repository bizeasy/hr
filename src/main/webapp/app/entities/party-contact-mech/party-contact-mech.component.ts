...
@Component({
  selector: 'sys-party-contact-mech',
...
export class PartyContactMechComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.partyContactMechService
      ...
      .subscribe(
        (res: HttpResponse<IPartyContactMech[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInPartyContactMeches(): void {
    this.eventSubscriber = this.eventManager.subscribe('partyContactMechListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IPartyContactMech[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/party-contact-mech'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
