...
export class GeoPointComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.geoPointService
      ...
      .subscribe(
        (res: HttpResponse<IGeoPoint[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInGeoPoints(): void {
    this.eventSubscriber = this.eventManager.subscribe('geoPointListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IGeoPoint[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/geo-point'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
