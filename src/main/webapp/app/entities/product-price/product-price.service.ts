...
export class ProductPriceService {
  ...
  protected convertDateFromClient(productPrice: IProductPrice): IProductPrice {
    const copy: IProductPrice = Object.assign({}, productPrice, {
      fromDate: productPrice.fromDate && productPrice.fromDate.isValid() ? productPrice.fromDate.toJSON() : undefined,
      thruDate: productPrice.thruDate && productPrice.thruDate.isValid() ? productPrice.thruDate.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((productPrice: IProductPrice) => {
        productPrice.fromDate = productPrice.fromDate ? moment(productPrice.fromDate) : undefined;
        productPrice.thruDate = productPrice.thruDate ? moment(productPrice.thruDate) : undefined;
      });
    }
    return res;
  }
