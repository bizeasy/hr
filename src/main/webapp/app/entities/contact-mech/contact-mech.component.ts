...
export class ContactMechComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.contactMechService
      ...
      .subscribe(
        (res: HttpResponse<IContactMech[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInContactMeches(): void {
    this.eventSubscriber = this.eventManager.subscribe('contactMechListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IContactMech[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/contact-mech'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
