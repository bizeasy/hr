...
export class InvoiceItemComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.invoiceItemService
      ...
      .subscribe(
        (res: HttpResponse<IInvoiceItem[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInInvoiceItems(): void {
    this.eventSubscriber = this.eventManager.subscribe('invoiceItemListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IInvoiceItem[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/invoice-item'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
