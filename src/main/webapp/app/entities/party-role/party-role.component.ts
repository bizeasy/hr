...
export class PartyRoleComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.partyRoleService
      ...
      .subscribe(
        (res: HttpResponse<IPartyRole[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInPartyRoles(): void {
    this.eventSubscriber = this.eventManager.subscribe('partyRoleListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IPartyRole[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/party-role'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
