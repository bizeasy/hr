...
@Component({
  selector: 'sys-work-effort-inventory-assign-update',
  templateUrl: './work-effort-inventory-assign-update.component.html',
})
export class WorkEffortInventoryAssignUpdateComponent implements OnInit {
  isSaving = false;
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ workEffortInventoryAssign }) => {
      this.updateForm(workEffortInventoryAssign);
  ...
  updateForm(workEffortInventoryAssign: IWorkEffortInventoryAssign): void {
    this.editForm.patchValue({
      id: workEffortInventoryAssign.id,
      quantity: workEffortInventoryAssign.quantity,
      workEffort: workEffortInventoryAssign.workEffort,
      inventoryItem: workEffortInventoryAssign.inventoryItem,
      status: workEffortInventoryAssign.status,
    });
  }
  ...
  save(): void {
    this.isSaving = true;
    const workEffortInventoryAssign = this.createFromForm();
    if (workEffortInventoryAssign.id !== undefined) {
      this.subscribeToSaveResponse(this.workEffortInventoryAssignService.update(workEffortInventoryAssign));
    } else {
      this.subscribeToSaveResponse(this.workEffortInventoryAssignService.create(workEffortInventoryAssign));
    }
  }

  private createFromForm(): IWorkEffortInventoryAssign {
    return {
      ...new WorkEffortInventoryAssign(),
  ...
  protected subscribeToSaveResponse(result: Observable<HttpResponse<IWorkEffortInventoryAssign>>): void {
    result.subscribe(
      () => this.onSaveSuccess(),
      () => this.onSaveError()
    );
