...
export class UomComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.uomService
      ...
      .subscribe(
        (res: HttpResponse<IUom[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IUom): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInUoms(): void {
    this.eventSubscriber = this.eventManager.subscribe('uomListModification', () => this.loadPage());
  }

  delete(uom: IUom): void {
  ...
  protected onSuccess(data: IUom[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
