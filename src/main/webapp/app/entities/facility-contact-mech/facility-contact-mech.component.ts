...
@Component({
  selector: 'sys-facility-contact-mech',
...
export class FacilityContactMechComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.facilityContactMechService
      ...
      .subscribe(
        (res: HttpResponse<IFacilityContactMech[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInFacilityContactMeches(): void {
    this.eventSubscriber = this.eventManager.subscribe('facilityContactMechListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IFacilityContactMech[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/facility-contact-mech'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.facilityContactMeches = data || [];
