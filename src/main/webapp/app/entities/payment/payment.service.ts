...
export class PaymentService {
  ...
  protected convertDateFromClient(payment: IPayment): IPayment {
    const copy: IPayment = Object.assign({}, payment, {
      effectiveDate: payment.effectiveDate && payment.effectiveDate.isValid() ? payment.effectiveDate.toJSON() : undefined,
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((payment: IPayment) => {
        payment.effectiveDate = payment.effectiveDate ? moment(payment.effectiveDate) : undefined;
