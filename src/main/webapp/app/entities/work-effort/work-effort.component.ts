...
export class WorkEffortComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.workEffortService
      ...
      .subscribe(
        (res: HttpResponse<IWorkEffort[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInWorkEfforts(): void {
    this.eventSubscriber = this.eventManager.subscribe('workEffortListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IWorkEffort[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/work-effort'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
