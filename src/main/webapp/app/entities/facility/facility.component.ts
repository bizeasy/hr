...
export class FacilityComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.facilityService
      ...
      .subscribe(
        (res: HttpResponse<IFacility[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInFacilities(): void {
    this.eventSubscriber = this.eventManager.subscribe('facilityListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IFacility[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
