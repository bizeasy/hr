...
export class GeoAssocComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.geoAssocService
      ...
      .subscribe(
        (res: HttpResponse<IGeoAssoc[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInGeoAssocs(): void {
    this.eventSubscriber = this.eventManager.subscribe('geoAssocListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IGeoAssoc[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/geo-assoc'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
